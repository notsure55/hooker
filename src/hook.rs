...
impl Hook {
    ...
    pub fn enable(&mut self) -> Result<()> {
        ...
        enable_hook(target)?;
        log::info!("Enabled hook {}", self.name);
    ...
    pub fn disable(&self) -> Result<()> {
        if let Some(target) = self.target {
            if !target.is_null() {
                disable_hook(target)?;
                log::info!("Disabled Hook {}", self.name);
            } else {
                return Err(anyhow!("Failed to disable hook for {}", self.name));
            }
        } else {
    ...
    pub fn remove(&self) -> Result<()> {
        if let Some(target) = self.target {
            if !target.is_null() {
                remove_hook(target)?;
                log::info!("Removed Hook {}", self.name);
            } else {
                return Err(anyhow!("Failed to remove hook for {}", self.name));
            }
        } else {
            log::warn!("Target for {} is {:?}", self.name, self.target);
        }
